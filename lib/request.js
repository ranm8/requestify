...
Request.prototype.getUri = function () {
    var parts = this.getUrlParts(),
        queryObject = (!parts || !parts.query) ? {} : queryString.parse(parts.query),
        query = '';

    if (this.params) {
        for (var i in this.params) {
            if (this.params.hasOwnProperty(i)) {
                queryObject[i] = this.params[i];
            }
        }
    }
...
Request.prototype.getHeaders = function () {
    ...
    for (var h in this.defaultHeaders) {
        if (this.defaultHeaders.hasOwnProperty(h)) {
            headers[h] = this.defaultHeaders[h];
        }
    }
    ...
    for (var j in requestedHeaders) {
        if (requestedHeaders.hasOwnProperty(j)) {
            headers[j] = requestedHeaders[j];
        }
    }
